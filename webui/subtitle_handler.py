...
def generate_preview_html(font, size, color, highlight, outline, outline_thick, shadow, shadow_sz, bold, italic, upper, 
    ...
    # Check RGB mode before sanitizing
    _rgb_preview = str(color).strip().upper() in ('RGB', 'RAINBOW')
    if _rgb_preview:
        color = '#FF0080'      # magenta as visual stand-in
    ...
    # Build RGB animation CSS if needed
    rgb_animation_style = ""
    rgb_span_extra = ""
    if _rgb_preview:
        rgb_animation_style = """
    ...
    <div style="
        ...
        margin-bottom: 10px;
        border: 1px solid {'#880088' if _rgb_preview else '#444'};
        display: flex;
        justify-content: center;
        align-items: center;
        min-height: 120px;
    ">
        <span class="{rgb_span_extra.strip()}" style="
            font-size: {base_preview_px}px;
            {'color: #ff0080;' if _rgb_preview else f'color: {color};'}
            font-weight: {weight};
            font-style: {style};
...
def render_preview_video(font, size, color, highlight, outline, outline_thick, shadow, shadow_sz, bold, italic, upper,
    ...
    # Detect RGB Rainbow mode
    _rgb_active = str(color).strip().upper() in ('RGB', 'RAINBOW')
